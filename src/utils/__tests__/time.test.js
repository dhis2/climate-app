...
describe('time utils', () => {
    ...
    describe('computeNextPeriod', () => {
        // computeNextPeriod figures out the next date range to import after a
        // previous run. It advances the start by one period unit beyond the
        // previous endTime, and caps the end at the last complete period (or
        // the dataset maximum, whichever is earlier). It returns null when
        ...
        it('returns null when there is no previous import', () => {
            expect(computeNextPeriod(null, {})).toBeNull()
            expect(computeNextPeriod(undefined, {})).toBeNull()
        })

        it('advances a DAILY period: next start is the day after the previous end, end is yesterday', () => {
            const lastImport = {
                endTime: '2024-01-15',
                periodType: DAILY,
                calendar: 'gregory',
            }
            const result = computeNextPeriod(lastImport, {})
            expect(result.startTime).toEqual('2024-01-16')
            expect(result.endTime).toEqual(yesterday)
        })

        it('advances a WEEKLY period: next start is 7 days after the previous end, end is 7 days ago', () => {
            const lastImport = {
                endTime: '2024-01-07',
                periodType: WEEKLY,
                calendar: 'gregory',
            ...
            const result = computeNextPeriod(lastImport, {})
            expect(result.startTime).toEqual('2024-01-14')
            expect(result.endTime).toEqual(lastWeekEnd)
        })

        it('advances a MONTHLY period: next start is the first of the following month, end is the last day of the previous month', () => {
            const lastImport = {
                endTime: '2024-06-30',
                periodType: MONTHLY,
                calendar: 'gregory',
            ...
            const result = computeNextPeriod(lastImport, {})
            expect(result.startTime).toEqual('2024-07-01')
            expect(result.endTime).toEqual(lastMonthEnd)
        })

        it('advances a MONTHLY period from December to January of the next year', () => {
            const lastImport = {
                endTime: '2024-12-31',
                periodType: MONTHLY,
                calendar: 'gregory',
            ...
            const result = computeNextPeriod(lastImport, {})
            expect(result.startTime).toEqual('2025-01-01')
            expect(result.endTime).toEqual(lastMonthEnd)
        })

        it('advances a YEARLY period: next start is the following year, end is last year', () => {
            const lastImport = {
                endTime: '2022',
                periodType: YEARLY,
                calendar: 'gregory',
            ...
            const result = computeNextPeriod(lastImport, {})
            expect(result.startTime).toEqual('2023')
            expect(result.endTime).toEqual(lastYear)
        })

        it('sets the end time to yesterday for DAILY (data for today does not yet exist)', () => {
            const lastImport = {
                endTime: '2024-01-15',
                periodType: DAILY,
                calendar: 'gregory',
        ...
        it('caps the end time at the dataset maximum when it is earlier than today', () => {
            const lastImport = {
                endTime: '2024-01-01',
                periodType: DAILY,
                calendar: 'gregory',
            ...
            const result = computeNextPeriod(lastImport, dataset)
        ...
        it('returns null when the previous import already reached yesterday (DAILY)', () => {
            // nextStart would be today, but endTime cap is yesterday — nothing left to import
            const lastImport = {
                endTime: yesterday,
                periodType: DAILY,
                calendar: 'gregory',
            }
            expect(computeNextPeriod(lastImport, {})).toBeNull()
        })

        it('returns null when the previous import already reached last week (WEEKLY)', () => {
            // nextStart would be today, but endTime cap is 7 days ago — nothing left to import
            const lastImport = {
                endTime: lastWeekEnd,
                periodType: WEEKLY,
                calendar: 'gregory',
        ...
        it('returns null when the previous import already reached the end of last month (MONTHLY)', () => {
            // nextStart would be the first of the current month, but endTime cap is end of last month
            const lastImport = {
        ...
        it('returns null when the previous import already covered last year (YEARLY)', () => {
            // nextStart would be the current year, but endTime cap is last year — nothing left to import
            const lastImport = {
                endTime: lastYear,
                periodType: YEARLY,
                calendar: 'gregory',
        ...
        it('returns null when the next start would exceed the dataset maximum', () => {
            const lastImport = {
                endTime: '2024-06-01',
                periodType: DAILY,
                calendar: 'gregory',
            ...
            expect(computeNextPeriod(lastImport, dataset)).toBeNull()
        })

        it('carries the calendar and periodType into the result', () => {
            const lastImport = {
                endTime: '2024-01-15',
                periodType: WEEKLY,
                calendar: 'nepali',
            ...
            expect(result.calendar).toEqual('nepali')
        })
    })
